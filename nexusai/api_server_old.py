...
class nexusaiAPIServer:
    ...
    def _build_app(self) -> FastAPI:
        app = FastAPI(title="nexusai Proxy")
        ...
        async def chat_completions(
            ...
            owner: nexusaiAPIServer = request.app.state.owner
    ...
    async def _forward_to_llm(self, body: dict[str, Any]) -> dict[str, Any]:
        ...
        if not api_base:
            raise HTTPException(
                status_code=503,
                detail="llm_api_base is not configured. Run 'nexusai setup' first.",
            )
    ...
    def _print_ready_banner(self):
        ...
        banner = (
            f"\n{'=' * 70}\n"
            f"  nexusai proxy ready  [mode={self.config.mode}]\n"
